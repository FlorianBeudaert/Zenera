...
import { FiltrePopover } from '@/components/FiltrePopover';
...
export default function WikiDatapackPage({ params }: { params: Promise<{ datapackId: string }> }) {
 ...
 const [selectedObtentionTypes, setSelectedObtentionTypes] = useState<string[]>([]);
 const [filterOpen, setFilterOpen] = useState(false);

 const crafters = useMemo(() => {
  if (!content) return [];
  const crafterSet = new Set<string>();
  Object.values(content).forEach(items => {
   items?.forEach(item => {
    item.obtention?.forEach(source => {
     if (source.type === ObtentionType.CRAFTER && source.crafterName) {
      crafterSet.add(source.crafterName);
     }
    });
   });
  });
  return Array.from(crafterSet).sort();
 }, [content]);

 const obtentionTypes = useMemo(() => {
  if (!content) return [];
  const set = new Set<string>();
  Object.values(content).forEach(items => {
   items?.forEach(item => {
    item.obtention?.forEach(source => {
     if (source.type && source.type !== ObtentionType.CRAFTER) {
      set.add(source.type);
     }
    });
   });
 ...
 return (
  <div className="min-h-screen bg-gradient-to-br from-primary/10 via-white/60 to-primary-hover/20 py-20 px-2">
   <div className="max-w-4xl mx-auto text-center mb-6">
    ...
    <div className="flex justify-center mt-8 items-center relative">
     <SearchBar
      value={search}
      onChange={setSearch}
      onFilterClick={() => setFilterOpen(o => !o)}
      filterActive={filterOpen || selectedCategories.length > 0 || selectedCrafters.length > 0 || selectedObtentionTypes.length > 0}
     />
     <div className="relative">
      <FiltrePopover
       ...
       onCraftersChange={setSelectedCrafters}
       obtentionTypes={obtentionTypes}
       selectedObtentionTypes={selectedObtentionTypes}
       onObtentionChange={setSelectedObtentionTypes}
       open={filterOpen}
   ...
   <div className="max-w-7xl mx-auto space-y-16">
    {Object.entries(content)
     .filter(([type]) =>
      selectedCategories.length === 0 || selectedCategories.includes(type)
     )
     .map(([type, items], categoryIndex, filteredCategories) => {
      const filteredItems = items?.filter(item => {
       const matchesSearch = item.name.toLowerCase().includes(search.toLowerCase());
       const matchesCrafter = selectedCrafters.length === 0 ||
        item.obtention?.some(source =>
         source.type === ObtentionType.CRAFTER &&
         source.crafterName &&
         selectedCrafters.includes(source.crafterName)
        );
       const matchesObtentionType = selectedObtentionTypes.length === 0 ||
        item.obtention?.some(source =>
         source.type && selectedObtentionTypes.includes(source.type)
        );
       return matchesSearch && matchesCrafter && matchesObtentionType;
      });
      ...
      return (
       <React.Fragment key={type}>
        <section>
         <h2 className="text-3xl font-bold text-primary mb-8 tracking-wide drop-shadow">
          {formatLabel(type)}
         </h2>
        ...
        {categoryIndex < filteredCategories.length - 1 && (
         <div className="flex items-center justify-center my-12">
          <div className="h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent w-full max-w-4xl"></div>
          <div className="mx-4 w-2 h-2 bg-primary/40 rounded-full flex-shrink-0"></div>
          <div className="h-px bg-gradient-to-r from-transparent via-primary/30 to-transparent w-full max-w-4xl"></div>
         </div>
        )}
       </React.Fragment>
